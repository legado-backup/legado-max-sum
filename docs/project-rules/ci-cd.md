...
> 注意：CI 中 lint 与格式检查（spotless）通过视为"完成"的一部分（本地 `:app:lintAppMaxDebug`、`compileDebugSources` 也要能过）。
...
| `lint.yaml`            | Quick Lint and Compile | push / pull_request                                    | commit 规范 + 编译 + spotless(Kotlin 格式) + Android lint + 帮助文档映射校验               |
